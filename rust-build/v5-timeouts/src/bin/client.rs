// EP 2 Scene 5 — v5: client that reacts to error responses
...
async fn main() {
    ...
    match call(&client, "add", json!([1, 2]), 5000).await {
        Ok(result) => println!("add(1,2) => {result}"),
        Err(e) => eprintln!("add failed: {e}"),
    }

    println!("hang() with 2s timeout...");
    match call(&client, "hang", json!([]), 2000).await {
        Ok(result) => println!("hang => {result}"),
        Err(e) => eprintln!("hang failed: {e}"),
    }
...
async fn call(client: &Client, method: &str, params: Value, timeout_ms: u64) -> Result<Value, Box<dyn std::error::Error>> {
    ...
    match tokio::time::timeout(Duration::from_millis(timeout_ms), work).await {
        Ok(inner) => inner,
        Err(_) => Err(format!("Timeout after {timeout_ms}ms (id={id})").into()),
    }
